 ...
 *
 * As a common driver, TTY accepts these MESSAGE:
 * 	|- DEV_OPEN
 * 	|- DEV_READ
 * 	|- DEV_WRITE
 *
 * Besides, it accepts the other two types of MESSAGE from clock_handler() and a PROC
 * (who is not FS):
 *
 * 	- HARD_INT  MESSAGE from clock_handler():
 * 		- Every time clock interrupt occurs, the clock handler will check whether any key
 * 		  has been pressed. If so, it'll invoke inform_int() to wake up TTY. It is a
 * 		  special message because it is not from a process -- clock handler is not a
 * 		  process.
 *
 * 	- TTY_WRITE MESSAGE from a PROC:
 * 		- TTY is a driver. In most cases MESSAGE is passed from a PROC to FS then to TTY.
 * 		  For some historical reason, PROC is allowed to pass a TTY_WRITE MESSAGE directly
 * 		  to TTY. Thus a PROC can write to a tty directly.
 *
 ...
 *					tty_dev_read
 **************************************************************************************************
 * Get chars from the keyboard buffer if the TTY::console if the 'current' console.
 *
...
PRIVATE void tty_do_read(TTY* tty, MESSAGE* msg){
	...
	tty->tty_req_buf	= (void*) va2la(tty->tty_procnr, msg->BUF);
...
PRIVATE void tty_do_write(TTY* tty, MESSAGE* msg){
	...
	while(i){
		...
		phys_copy((void*) va2la(TASK_TTY, buf), (void*) p, bytes);
		for(j=0; j<bytes; j++){
 ...
 * To deal with the metadata of the key value.
 *
 ...
 *					sys_printx
 **************************************************************************************************
 * System calls accept four parameters. 'printx' needs only two, so it wastes the other two.
 *
 * @note	'printx' accepts only one parameter -- 'char* s'
 * 		the other one -- 'PROCESS* p_proc' is pushed by kernel.asm::sys_call so that the
...
PUBLIC int sys_printx(int _unused1, int _unused2, char* s, PROCESS* p_proc){
	...
	if(k_reenter == 0){		/* printx() called in ring 1~3 */
		p = (void*) va2la(proc2pid(p_proc), s);
